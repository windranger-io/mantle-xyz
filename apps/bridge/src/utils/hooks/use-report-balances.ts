...
const mantleTestnetETH = getTokenBySymbol(MANTLE_TESTNET_CHAIN_ID, TokenSymbol.ETH);
  ...
  React.useEffect(() => {
    try {
      ...
      const reportBalances = async () => {
        ...
        const ETH = new Contract(mantleTestnetETH.address, erc20ABI, mantleTestnetProvider);
        const l2Balance = await ETH.balanceOf(crossChainMessenger.l2Signer.getAddress());

        // eslint-disable-next-line max-len, no-console
        console.log(`On L1:${formatUnits(l1Balance, mantleTestnetETH.decimals)} On L2:${formatUnits(l2Balance, mantleTestnetETH.decimals)}`);
      };
