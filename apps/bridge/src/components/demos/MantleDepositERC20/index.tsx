...
function MantleDepositERC20({
  ...
  const {
    ...
    mutationFn: async (depositERC20Amount: number) => {
      ...
      const bigDepositERC20Amount = parseUnits(depositERC20Amount.toString(), goerliERC20Token.decimals);
      ...
      const response =
        await crossChainMessenger.depositERC20(
          goerliERC20Token.address,
          mantleTestnetERC20Token.address,
          bigDepositERC20Amount
        );
      ...
      // eslint-disable-next-line no-console
      console.log(`depositERC20 took ${(new Date().getTime() - startTime) / 1000} seconds\n\n`);
    },
    onError: error => {
  ...
  return (
    <div>
      ...
      <BasicButton
        ...
        {`${depositERC20MutateIsLoading ? 'Depositing' : 'Deposit'} ${tokenAmount} ERC20 (${tokenSymbol})`}
      </BasicButton>
