...
function MantleWithdrawERC20({
  ...
  const mantleTestnetERC20Token = getTokenBySymbol(MANTLE_TESTNET_CHAIN_ID, tokenSymbol);
  ...
  const {
    ...
    mutationFn: async (withdrawERC20Amount: number) => {
      ...
      const bigWithdrawERC20Amount = parseUnits(withdrawERC20Amount.toString(), mantleTestnetERC20Token.decimals);

      const response =
        await crossChainMessenger.withdrawERC20(
      ...
      // eslint-disable-next-line no-console
      console.log(`withdrawal took ${(new Date().getTime() - startTime) / 1000} seconds\n\n`);
    },
    onError: error => {
  ...
  const handleWithdrawERC20 = () => {
    if (connectedChainID !== MANTLE_TESTNET_CHAIN.id) {
      alert(`You need to switch to ${MANTLE_TESTNET_CHAIN.name} in order to withdraw!`);
      return;
    }
  ...
  return (
    <div>
      ...
      <BasicButton
        ...
        {`${withdrawERC20MutateIsLoading ? 'Withdrawing' : 'Withdraw'} ${tokenAmount} ERC20 (${tokenSymbol})`}
      </BasicButton>
      {withdrawERC20MutateIsError ? (
        <div>An error occurred: {getErrorMessage(withdrawERC20MutateError)}</div>
      ) : null}
      {withdrawERC20MutateIsSuccess ? <div>{tokenAmount} withdrawn!</div> : null}
    </div>
  );
}
