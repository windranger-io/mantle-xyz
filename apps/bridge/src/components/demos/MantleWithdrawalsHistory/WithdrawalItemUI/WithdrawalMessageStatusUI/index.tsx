...
function WithdrawalMessageStatusUI({
  item,
  withdrawalsRefetch
}: Props) {
  ...
  if (!item.batch) {
    return (
      <span>{WithdrawalStatus.Pending}</span>
    );
  } if (Date.now() - 1000 * item.blockTimestamp < challengePeriod) {
    return (
      <span>{WithdrawalStatus.ChallengePeriod}</span>
    );
    ...
    return (
      <span>{WithdrawalStatus.Complete}</span>
    );
