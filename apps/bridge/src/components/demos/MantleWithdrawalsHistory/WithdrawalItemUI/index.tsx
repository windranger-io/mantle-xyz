...
function WithdrawalItemUI({
  item,
  withdrawalsRefetch
}: Props) {
  const timeLabel = new Date(Number(item.blockTimestamp) * 1000).toLocaleString();
  const tokenAddress = item.l2Token;
  const token = getTokenByAddress(MANTLE_TESTNET_CHAIN.id, tokenAddress);
  const amountLabel = `${formatUnits(item.amount, token.decimals)} ${token.symbol}`;
