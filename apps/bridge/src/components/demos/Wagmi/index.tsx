...
function Wagmi() {
  ...
  const handleConnect = async (connector: Connector) => {
    try {
      const { provider, chain } = await connectAsync({ connector })
  ...
  if (isConnected) {
    return (
      <div>
        ...
        <BasicButton onClick={() => disconnect()}>Disconnect</BasicButton>
      </div>
    )
  }
  ...
  return (
    <div>
      <ul className={clsx('max-w-xs', 'space-y-4')}>
        {connectors.map(item => (
          <li key={item.id}>
            <BasicButton
              className={clsx('w-full')}
              disabled={!item.ready || isLoading}
